...
class UiAtlas(object):
    ...
    This class can be used to control the part of displayed surroundings (both / left
    ...
    def __init__(self):
        ...
        #######################################################################
        # Visible :
        self._engram_grp.clicked.connect(self._fcn_engram_visible)
        # Template :
        all_templates = self.atlas.list()
        enable_template = self.atlas.name in all_templates
        if enable_template:
            self._engram_template.addItems(all_templates)
            idx = all_templates.index(self.atlas.name)
            self._engram_template.setCurrentIndex(idx)
            self._engram_template.currentIndexChanged.connect(
                self._fcn_engram_template)
            # Hemisphere :
            idx = ['both', 'left', 'right'].index(self.atlas.mesh.hemisphere)
            self._engram_hemi.setCurrentIndex(idx)
            self._engram_hemi.currentIndexChanged.connect(
                self._fcn_engram_hemisphere)
        self._engram_template.setEnabled(enable_template)
        self._engram_hemi.setEnabled(enable_template)
        # Translucent :
        self._engram_translucent.setChecked(self.atlas.translucent)
        self._engram_translucent.clicked.connect(self._fcn_engram_translucent)
        self._engram_alpha.valueChanged.connect(self._fcn_engram_alpha)
        # Slices :
        self._fcn_engram_reset_slider()
        self._engram_xmin.valueChanged.connect(self._fcn_engram_slices)
        self._engram_xmax.valueChanged.connect(self._fcn_engram_slices)
        self._engram_ymin.valueChanged.connect(self._fcn_engram_slices)
        self._engram_ymax.valueChanged.connect(self._fcn_engram_slices)
        self._engram_zmin.valueChanged.connect(self._fcn_engram_slices)
        self._engram_zmax.valueChanged.connect(self._fcn_engram_slices)
        # Light :
        self._engram_inlight.clicked.connect(self._fcn_engram_inlight)
    ...
    ###########################################################################
    ###########################################################################
    def _fcn_engram_visible(self):
        """Display / hide the engram."""
        self.menuDispEngram.setChecked(self._engram_grp.isChecked())
        self._fcn_menu_disp_engram()

    def _fcn_engram_template(self):
        """Control the type of engram to use."""
        # _____________________ TEMPLATE _____________________
        template = str(self._engram_template.currentText())
        hemisphere = str(self._engram_hemi.currentText())
    ...
    def _fcn_engram_hemisphere(self):
        """Change the hemisphere."""
        hemi = str(self._engram_hemi.currentText())
        self.atlas.mesh.hemisphere = hemi

    def _fcn_engram_translucent(self):
        """Use translucent or opaque engram."""
        viz = self._engram_translucent.isChecked()
        self.atlas.translucent = viz
        self._engram_alpha.setEnabled(viz)
        self._fcn_engram_alpha()

    def _fcn_engram_alpha(self):
        """Update engram transparency."""
        alpha = self._engram_alpha.value() / 100.
        self.atlas.alpha = alpha

    def _fcn_engram_reset_slider(self):
        ...
        # xmin
        self._engram_xmin.setMinimum(xmin)
        self._engram_xmin.setMaximum(xmax)
        self._engram_xmin.setSingleStep((xmin - xmax) / n_cut)
        self._engram_xmin.setValue(xmin)
        # xmax
        self._engram_xmax.setMinimum(xmin)
        self._engram_xmax.setMaximum(xmax)
        self._engram_xmax.setSingleStep((xmin - xmax) / n_cut)
        self._engram_xmax.setValue(xmax)
        # ymin
        self._engram_ymin.setMinimum(ymin)
        self._engram_ymin.setMaximum(ymax)
        self._engram_ymin.setSingleStep((ymin - ymax) / n_cut)
        self._engram_ymin.setValue(ymin)
        # ymax
        self._engram_ymax.setMinimum(ymin)
        self._engram_ymax.setMaximum(ymax)
        self._engram_ymax.setSingleStep((ymin - ymax) / n_cut)
        self._engram_ymax.setValue(ymax)
        # zmin
        self._engram_zmin.setMinimum(zmin)
        self._engram_zmin.setMaximum(zmax)
        self._engram_zmin.setSingleStep((zmin - zmax) / n_cut)
        self._engram_zmin.setValue(zmin)
        # zmax
        self._engram_zmax.setMinimum(zmin)
        self._engram_zmax.setMaximum(zmax)
        self._engram_zmax.setSingleStep((zmin - zmax) / n_cut)
        self._engram_zmax.setValue(zmax)

    def _fcn_engram_slices(self):
        """Slice the engram."""
        self.atlas.mesh.xmin = float(self._engram_xmin.value())
        self.atlas.mesh.xmax = float(self._engram_xmax.value())
        self.atlas.mesh.ymin = float(self._engram_ymin.value())
        self.atlas.mesh.ymax = float(self._engram_ymax.value())
        self.atlas.mesh.zmin = float(self._engram_zmin.value())
        self.atlas.mesh.zmax = float(self._engram_zmax.value())
        self.atlas.mesh.update()

    def _fcn_engram_inlight(self):
        """Set light to be inside the engram."""
        self.atlas.mesh.inv_light = self._engram_inlight.isChecked()
        self.atlas.mesh.update()
